    ...
    <div className='bg-gray-4 relative max-h-screen'>
        ...
        <div className='flex flex-col h-[calc(100vh-49px)]'>
            <div className='p-2 flex-1 overflow-y-scroll dropdownscrollbar'>
                <button onClick={() => {handleDmButton('friends')}} className={activeDmButton === 'friends' ? 'flex items-center px-3 font-medium text-white h-[42px] w-full rounded bg-gray-7' : 'group hover:text-white hover:bg-gray-7 flex items-center px-3 font-medium h-[42px] text-gray-3 w-full rounded'}>
                    <svg className="mr-4" aria-hidden="true" role="img" width="24" height="24" viewBox="0 0 24 24"><g fill="none" fillRule="evenodd"><path className={activeDmButton === 'friends' ? "fill-white" : "group-hover:fill-white fill-gray-3"} fillRule="nonzero" d="M0.5,0 L0.5,1.5 C0.5,5.65 2.71,9.28 6,11.3 L6,16 L21,16 L21,14 C21,11.34 15.67,10 13,10 C13,10 12.83,10 12.75,10 C8,10 4,6 4,1.5 L4,0 L0.5,0 Z M13,0 C10.790861,0 9,1.790861 9,4 C9,6.209139 10.790861,8 13,8 C15.209139,8 17,6.209139 17,4 C17,1.790861 15.209139,0 13,0 Z" transform="translate(2 4)"></path><path d="M0,0 L24,0 L24,24 L0,24 L0,0 Z M0,0 L24,0 L24,24 L0,24 L0,0 Z M0,0 L24,0 L24,24 L0,24 L0,0 Z"></path></g></svg>
                    Friends
                </button>
                ...
                return(
                    <div key={crypto.randomUUID()} className={activeDmButton === `${item}` ? 'text-white bg-gray-7 group flex items-center font-medium h-[42px] w-full rounded mb-1' : 'group hover:text-white hover:bg-gray-5 text-gray-3 flex items-center mb-1 font-medium h-[42px] w-full rounded'}>
                        ...
                        <button onClick={() => {closeConversationButton(item)}} key={crypto.randomUUID()} className='pr-2'>
                            <div key={crypto.randomUUID()} className={activeDmButton === `${item}` ? 'w-4 h-4 opacity-70 m-0.5 pr-2 hidden group-hover:block hover:opacity-100' : 'w-4 h-4 opacity-70 m-0.5 pr-2 hidden group-hover:block hover:opacity-100'}>
